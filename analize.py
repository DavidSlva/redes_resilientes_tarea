...
API_TOKEN = "33fe5453699ceb"  # Reemplaza con tu token de ipinfo.io u otro servicio si es necesario

# Crear directorios principales
os.makedirs(RESULTADOS_DIR, exist_ok=True)
os.makedirs(ANALISIS_DIR, exist_ok=True)

# Crear subdirectorios para cada DEST_IP en resultados y analice
...
def fetch_metadata(ip):
    """
    Obtiene metadata para una IP utilizando la API de ipinfo.io.
    Puedes reemplazar esta función para usar otra API de tu preferencia.
    """
    try:
...
def generate_metadata_table(dest_ip, method, hops):
    ...
    # Directorio específico para la DEST_IP en resultados
    resultados_dest_dir = os.path.join(RESULTADOS_DIR, dest_ip)
    
    # Archivos de salida
    metadata_csv = os.path.join(resultados_dest_dir, f"tabla_metadata_{method}.csv")
    metadata_graph = os.path.join(resultados_dest_dir, f"tabla_metadata_{method}.png")
...
def plot_hops_per_method(dest_ip, routes):
    ...
    plt.tight_layout()
    hops_graph_file = os.path.join(RESULTADOS_DIR, dest_ip, f"hops_por_metodo_{dest_ip}.png")
...
def plot_as_distribution(dest_ip, method, hops):
    ...
    as_graph_file = os.path.join(RESULTADOS_DIR, dest_ip, f"distribucion_as_{dest_ip}.png")
